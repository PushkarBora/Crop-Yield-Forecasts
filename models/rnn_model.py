...
class RNNModel(nn.Module):
    """
    Vanilla RNN for time-series regression.
    ...
    def __init__(self, n_features, hidden_size, num_layers,
                 dropout_rnn, dropout_fc, nonlinearity="tanh"):
        super().__init__()
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout_fc),       # extra dropout for MC sampling
...
def auto_tune_rnn(
    ...
    log("\n🔍 Starting RNN Grid Search Auto-Tuning")
    ...
    grid = {
        ...
        "hidden_size":   irange(params["rnn_hidden_size_min"],
                                params["rnn_hidden_size_max"],
                                params["rnn_hidden_size_step"]),
        "num_layers":    irange(params["rnn_num_layers_min"],
                                params["rnn_num_layers_max"],
                                params["rnn_num_layers_step"]),
        "dropout_rnn":   frange(params["rnn_dropout_rnn_min"],
                                params["rnn_dropout_rnn_max"],
                                params["rnn_dropout_rnn_step"], 3),
        "dropout_fc":    frange(params["rnn_dropout_fc_min"],
                                params["rnn_dropout_fc_max"],
                                params["rnn_dropout_fc_step"], 3),
        "nonlinearity":  ["tanh", "relu"],
        "learning_rate": frange(params["rnn_learning_rate_min"],
                                params["rnn_learning_rate_max"],
                                params["rnn_learning_rate_step"], 6),
        "weight_decay":  frange(params["rnn_weight_decay_min"],
                                params["rnn_weight_decay_max"],
                                params["rnn_weight_decay_step"], 7),
        "huber_delta":   frange(params["rnn_huber_delta_min"],
                                params["rnn_huber_delta_max"],
                                params["rnn_huber_delta_step"], 2),
        "patience":      irange(params["rnn_patience_min"],
                                params["rnn_patience_max"],
                                params["rnn_patience_step"]),
        "epochs":   irange(params["rnn_epochs_min"],
                 params["rnn_epochs_max"],
                 params["rnn_epochs_step"]),
    }
...
# ============================================================
def _forecast_only_rnn(params, horizon, future_rain, future_mean_t):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    for path in ("static/rnn_bundle.pt", "static/rnn_weights.pt"):
    ...
    model = RNNModel(
        ...
        dropout_rnn  = bundle["dropout_rnn"],
        dropout_fc   = bundle["dropout_fc"],
        nonlinearity = bundle["nonlinearity"],
    ).to(device)
...
def run_rnn(
    ...
    """
    RNN model — lag-based, raw values, no log differencing.
    Mirrors the LSTM/GRU implementation exactly.
    ...
    if mode == "forecast":
        ...
        return _forecast_only_rnn(params, horizon, future_rain, future_mean_t)
    ...
    # Hyperparameters
    # ------------------------------------------------------------------
    LAGS         = params.get("rnn_lags") or params.get("lags", 3)
    split        = params.get("split", 0.85)
    use_tune     = params.get("auto_tune_rnn", True)

    hidden_size  = params.get("hidden_size",   8)
    num_layers   = params.get("num_layers",    2)
    dropout_rnn  = params.get("dropout_rnn",   0.0)
    ...
    # Variable order: target first, then exog (mirrors LSTM/GRU)
    ...
    # ------------------------------------------------------------------
    if use_tune:
        log("\n🔧 Auto-tuning RNN hyperparameters...")

        lags_values = list(range(
            params.get("rnn_lags_min", 2),
            params.get("rnn_lags_max", 5) + 1,
            params.get("rnn_lags_step", 1),
        ))
        ...
        for lag_val in lags_values:
            ...
            model_tmp = RNNModel(
                ...
                dropout_rnn=best_p["dropout_rnn"],
                dropout_fc=best_p["dropout_fc"],
                nonlinearity=best_p["nonlinearity"],
            ).to(device)
    ...
    ax.set_title("RNN — Training & Testing Set")
    ax.set_xlabel(time_col); ax.set_ylabel(target_col)
    ax.legend()
    plt.tight_layout()
    plt.savefig("static/rnn_train.png", dpi=120)
    plt.savefig("static/rnn_test.png",  dpi=120)
    plt.close()
