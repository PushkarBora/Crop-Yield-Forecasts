...
def auto_tune_rnn(
    ...
    """
    Auto-tune RNN using USER-DEFINED grid search ranges from params.
    """
    import itertools
    import numpy as np
    import torch
    ...
    log("\n🔍 Starting RNN Grid Search Auto-Tuning (USER RANGES)")
    
    # ============================================================
    # READ USER-DEFINED RANGES - USE WHILE LOOP TO FIX FLOATING POINT
    # ============================================================
    hidden_size_values = list(range(
        params["rnn_hidden_size_min"],
        params["rnn_hidden_size_max"] + 1,
        params["rnn_hidden_size_step"]
    ))
    
    num_layers_values = list(range(
        params["rnn_num_layers_min"],
        params["rnn_num_layers_max"] + 1,
        params["rnn_num_layers_step"]
    ))
    
    # ✅ FIX: Use while loop to avoid floating point errors
    dropout_rnn_values = []
    val = params["rnn_dropout_rnn_min"]
    while val <= params["rnn_dropout_rnn_max"] + 1e-9:
        dropout_rnn_values.append(round(val, 3))
        val += params["rnn_dropout_rnn_step"]
    
    dropout_fc_values = []
    val = params["rnn_dropout_fc_min"]
    while val <= params["rnn_dropout_fc_max"] + 1e-9:
        dropout_fc_values.append(round(val, 3))
        val += params["rnn_dropout_fc_step"]
    
    learning_rate_values = []
    val = params["rnn_learning_rate_min"]
    while val <= params["rnn_learning_rate_max"] + 1e-9:
        learning_rate_values.append(round(val, 6))
        val += params["rnn_learning_rate_step"]
    
    weight_decay_values = []
    val = params["rnn_weight_decay_min"]
    while val <= params["rnn_weight_decay_max"] + 1e-9:
        weight_decay_values.append(round(val, 7))
        val += params["rnn_weight_decay_step"]
    
    huber_delta_values = []
    val = params["rnn_huber_delta_min"]
    while val <= params["rnn_huber_delta_max"] + 1e-9:
        huber_delta_values.append(round(val, 2))
        val += params["rnn_huber_delta_step"]
    
    patience_values = list(range(
        params["rnn_patience_min"],
        params["rnn_patience_max"] + 1,
        params["rnn_patience_step"]
    ))
    
    # Nonlinearity (fixed options)
    ...
    param_grid = {
        ...
        'dropout_rnn': dropout_rnn_values,
        'dropout_fc': dropout_fc_values,
    ...
    log(f"   dropout_rnn: {dropout_rnn_values}")
    log(f"   dropout_fc: {dropout_fc_values}")
    log(f"   nonlinearity: {nonlinearity_values}")
...
class RNNModel(nn.Module):
    def __init__(
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),               # implicit activation (fixed)
...
def _forecast_only_rnn(
    ...
    bundle = torch.load("static/rnn_bundle.pt", weights_only=False)
    ...
    model = RNNModel(
        ...
        dropout_rnn=bundle["dropout_rnn"],  # ✅ CORRECT
        dropout_fc=bundle["dropout_fc"],    # ✅ CORRECT
        nonlinearity=bundle["nonlinearity"],  # ✅ CORRECT
    ).to(device)
    
    model.load_state_dict(
        torch.load("static/rnn_weights.pt", map_location=device, weights_only=False)
    )
    model.eval()
...
#DEFAULT + USER-OVERRIDABLE HYPERPARAMETERS
    # ===============================
    # RNN HYPERPARAMETERS (defaults)
    # ===============================
    ...
    use_auto_tune = params.get("auto_tune_rnn", True)
    # Architecture
    hidden_size = params.get("hidden_size", 8)
    num_layers = params.get("num_layers", 2)
    dropout_rnn = params.get("dropout_rnn", 0.2)
...
#Train–Test Split
    ...
    # Override with best parameters
        ...
        dropout_rnn = best_params_rnn['dropout_rnn']
        dropout_fc = best_params_rnn['dropout_fc']
        nonlinearity = best_params_rnn['nonlinearity']
        lr = best_params_rnn['learning_rate']
        weight_decay = best_params_rnn['weight_decay']
        huber_delta = best_params_rnn['huber_delta']
        patience = best_params_rnn['patience']

        log(f"✅ Using optimized RNN parameters")
...
#Metrics
    ...
    if True:  # always save after training
        torch.save(model.state_dict(), "static/rnn_weights.pt")
...
# -------------------------
    ...
    ax.set_title("RNN Model - Training Set")
    ax.set_xlabel("Year")
    ax.set_ylabel("Yield")
    ax.legend()

    fig.tight_layout()
    fig.savefig("static/rnn_train.png", dpi=150)
    plt.close(fig)
...
# -------------------------
    ...
    ax.set_title("RNN Model - Testing Set")
    ax.set_xlabel("Year")
    ax.set_ylabel("Yield")
    ax.legend()

    fig.tight_layout()
    fig.savefig("static/rnn_test.png", dpi=150)
    plt.close(fig)
