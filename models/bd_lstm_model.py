...
class BidirectionalLSTMModel(nn.Module):
    ...
    Architecture
    ────────────
    • One nn.LSTM with bidirectional=True and num_layers stacked layers.
    • The forward and backward final hidden states are concatenated →
      effective hidden dimension = 2 × hidden_size.
    • LayerNorm is applied on the concatenated representation for
      training stability.
    • FC head: Linear → ReLU → Dropout (MC-Dropout) → Linear → scalar.
    ...
    Why Bidirectional?
    ──────────────────
    A standard LSTM only reads the sequence left-to-right (past → present).
    A BiLSTM also reads right-to-left, allowing each time-step to see
    information from future lags in the input window before making a
    decision — useful when lag patterns carry useful context from both
    ...
    def __init__(self, n_features, hidden_size, num_layers, dropout):
        ...
        # LayerNorm on the concatenated forward+backward representation
    ...
    def forward(self, x):
        ...
        out, _ = self.bilstm(x)          # (batch, seq_len, 2*hidden_size)

        # Take last time-step output (contains info from entire sequence
        # in both forward and backward directions)
        last = out[:, -1, :]             # (batch, 2*hidden_size)
        last = self.layer_norm(last)     # stabilise deep stacks
...
def auto_tune_bd_lstm(
    ...
    log("\n🔍 Starting Bidirectional LSTM Grid Search Auto-Tuning")
    ...
    grid = {
        "hidden_size":  irange(params["bdlstm_hidden_size_min"],
                               params["bdlstm_hidden_size_max"],
                               params["bdlstm_hidden_size_step"]),
        "num_layers":   irange(params["bdlstm_num_layers_min"],
                               params["bdlstm_num_layers_max"],
                               params["bdlstm_num_layers_step"]),
        "dropout":      frange(params["bdlstm_dropout_min"],
                               params["bdlstm_dropout_max"],
                               params["bdlstm_dropout_step"], 3),
        "learning_rate": frange(params["bdlstm_learning_rate_min"],
                                params["bdlstm_learning_rate_max"],
                                params["bdlstm_learning_rate_step"], 6),
        "weight_decay": frange(params["bdlstm_weight_decay_min"],
                               params["bdlstm_weight_decay_max"],
                               params["bdlstm_weight_decay_step"], 7),
        "huber_delta":  frange(params["bdlstm_huber_delta_min"],
                               params["bdlstm_huber_delta_max"],
                               params["bdlstm_huber_delta_step"], 2),
        "patience":     irange(params["bdlstm_patience_min"],
                               params["bdlstm_patience_max"],
                               params["bdlstm_patience_step"]),
        "epochs":       irange(params["bdlstm_epochs_min"],
                               params["bdlstm_epochs_max"],
                               params["bdlstm_epochs_step"]),
    }
...
# ============================================================
def _forecast_only_bd_lstm(params, horizon, future_rain, future_mean_t):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    for path in ("static/bd_lstm_bundle.pt", "static/bd_lstm_weights.pt"):
        if not os.path.exists(path):
            raise FileNotFoundError(
                f"{path} not found. Please train the model first."
            )

    bundle = torch.load("static/bd_lstm_bundle.pt", weights_only=False)
    ...
    model.load_state_dict(
        torch.load("static/bd_lstm_weights.pt",
...
def run_bd_lstm(
    ...
    """
    Bidirectional LSTM — reads the input sequence in both directions,
    concatenates forward + backward hidden states, applies LayerNorm,
    then maps to a scalar forecast via an FC head with MC-Dropout.
    ...
    BD LSTM-specific params keys
    ────────────────────────────
    bdlstm_lags                 : sequence length
    bdlstm_lags_min/max/step    : lags grid-search range
    bdlstm_hidden_size          : hidden units (effective output = 2×)
    bdlstm_num_layers           : stacked BiLSTM layers
    bdlstm_dropout              : dropout (inter-layer + FC head)
    bdlstm_learning_rate        : Adam LR
    bdlstm_weight_decay         : Adam weight decay
    bdlstm_huber_delta          : Huber loss delta
    bdlstm_patience             : early-stopping patience
    bdlstm_epochs               : max training epochs
    auto_tune_bd_lstm           : bool — run grid search if True
    """
    ...
    if mode == "forecast":
        ...
        return _forecast_only_bd_lstm(params, horizon, future_rain, future_mean_t)
    ...
    # Hyperparameters
    # ------------------------------------------------------------------
    LAGS        = params.get("bdlstm_lags") or params.get("lags", 3)
    split       = params.get("split", 0.85)
    use_tune    = params.get("auto_tune_bd_lstm", True)

    hidden_size = params.get("bdlstm_hidden_size",    8)
    num_layers  = params.get("bdlstm_num_layers",     2)
    dropout     = params.get("bdlstm_dropout",        0.3)
    lr          = params.get("bdlstm_learning_rate",  0.003)
    weight_decay= params.get("bdlstm_weight_decay",   1e-4)
    epochs      = params.get("bdlstm_epochs",         200)
    patience    = params.get("bdlstm_patience",       30)
    huber_delta = params.get("bdlstm_huber_delta",    1.0)
    ...
    # ------------------------------------------------------------------
    if use_tune:
        log("\n🔧 Auto-tuning Bidirectional LSTM hyperparameters...")

        lags_values = list(range(
            params.get("bdlstm_lags_min",  2),
            params.get("bdlstm_lags_max",  5) + 1,
            params.get("bdlstm_lags_step", 1),
        ))
        ...
        for lag_val in lags_values:
            ...
            model_tmp = BidirectionalLSTMModel(
                n_features  = nv,
    ...
    torch.save(model.state_dict(), "static/bd_lstm_weights.pt")
    torch.save({
        # Architecture
        "n_features"  : n_v,
        "hidden_size" : hidden_size,
    ...
    ax.set_title("Bidirectional LSTM — Training & Testing Set")
    ax.set_xlabel(time_col)
    ax.set_ylabel(target_col)
    ax.legend()
    plt.tight_layout()
    plt.savefig("static/bd_lstm_train.png", dpi=120)
    plt.savefig("static/bd_lstm_test.png",  dpi=120)
    plt.close()
    ...
    model_config = {
        "hidden_size"          : hidden_size,
        "effective_output_dim" : 2 * hidden_size,   # forward + backward
        "num_layers"           : num_layers,
        "dropout"              : dropout,
        "learning_rate"        : lr,
