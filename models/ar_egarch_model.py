...
def _param_estimates_df(fit):
    ...
    Works for EGARCH, AR-EGARCH, ARX-EGARCH.
    """
    try:
        ci = fit.conf_int()          # columns: ['lower ...', 'upper ...']
...
def _info_criteria(fit):
    ...
    # Log-Likelihood  (arch uses .loglikelihood, not .llf)
    try:
    ...
    # AICc  — manual formula: AIC + 2k(k+1)/(n-k-1)
    try:
...
# ============================================================
def _bootstrap_forecast_aregarch(
    egarch_fit_full,
    future_exog_scaled,
    horizon,
    diff_order,
    last_val_before_diff,
    residuals_std,
    n_boot=200,
):
    """
    Resamples standardised residuals × EGARCH conditional volatility
    to produce empirical prediction intervals.
    """
    random.seed(SEED)
    np.random.seed(SEED)
    ...
    for _ in range(n_boot):
        try:
            fc_result = egarch_fit_full.forecast(
                horizon=horizon,
                x=future_exog_scaled,
                reindex=False,
            )
            mean_fc = fc_result.mean.values.flatten()[:horizon]
            var_fc  = fc_result.variance.values.flatten()[:horizon]
        except Exception:
            mean_fc = np.zeros(horizon)
            var_fc  = np.ones(horizon)

        noises = np.random.choice(residuals_std, size=horizon, replace=True)
        preds  = mean_fc + noises * np.sqrt(np.maximum(var_fc, 0.0))

        if diff_order == 1 and last_val_before_diff is not None:
            preds = last_val_before_diff + np.cumsum(preds)
    ...
    lower_95 = mean_pred - 1.960 * std_pred
    upper_95 = mean_pred + 1.960 * std_pred
    lower_80 = mean_pred - 1.282 * std_pred
    upper_80 = mean_pred + 1.282 * std_pred

    return mean_pred, lower_95, upper_95, lower_80, upper_80


# ============================================================
# AUTO-TUNE — grid search over ar_lags + EGARCH(p, o, q)
# ============================================================
def auto_tune_aregarch(train_series, exog_train, has_exogenous,
    ...
    log("\n🔍 Running Auto AR-EGARCH grid search (ar_lags, p, o, q)...")
...
def _arch_lm_test(series, lags=5, log=None):
    ...
    try:
        # Step 1 — select order using AICc + KPSS (matches R defaults)
        ...
        p, d, q     = order
        residuals   = pd.Series(arima_fit.resid()).iloc[1:].reset_index(drop=True)  # ← matches app.py
        arima_order = order

    except Exception as e:
        # NO OLS fallback — if Auto-ARIMA fails, test is inconclusive
        if log:
            log(f"\n⚠️  Auto-ARIMA failed: {str(e)}")
            log("   Cannot perform ARCH LM test without ARIMA residuals.")
            log("   Treating as NOT significant — EGARCH will not be fitted.")
        return 0.0, 1.0, False   # p=1.0 → not significant → no EGARCH
    ...
    if log:
        ...
        log(f"   Mean model   : Auto-ARIMA{arima_order} residuals")
        log(f"   ARCH lags    : {lags}")
        log(f"   H0 : No ARCH effects in residuals")
        log(f"   H1 : ARCH effects present → EGARCH is appropriate")
        log(f"   LM Statistic : {lm_stat:.4f}")
        log(f"   p-value      : {p_value:.4f}")
        log(f"   F-Statistic  : {f_stat:.4f}")
        log(f"   F p-value    : {f_p_value:.4f}")
        if p_value < 0.05:
            log(f"   ✅ ARCH effects detected (p={p_value:.4f} < 0.05) → Proceed with EGARCH")
        else:
            log(f"   ❌ No ARCH effects (p={p_value:.4f} ≥ 0.05) → EGARCH NOT appropriate")
        log("=" * 60)
...
# FORECAST-ONLY
# ============================================================
def _forecast_only_aregarch(params, horizon):
    for path in ("static/aregarch_bundle.pkl", "static/aregarch_model.pkl"):
        if not os.path.exists(path):
            raise FileNotFoundError(
                f"{path} not found. Please train the AR-EGARCH model first."
            )

    with open("static/aregarch_bundle.pkl", "rb") as f:
        bundle = pickle.load(f)
    with open("static/aregarch_model.pkl", "rb") as f:
        egarch_fit_full = pickle.load(f)
    ...
    residuals_std  = bundle.get("residuals",      np.array([0.0]))
    ar_lags        = bundle.get("ar_lags",        1)
    egarch_p       = bundle.get("egarch_p",       1)
    o_order        = bundle.get("egarch_o",       1)
    egarch_q       = bundle.get("egarch_q",       1)
    ...
    if has_exogenous:
        ...
        try:
            ...
            var_fc = np.full(
                horizon,
                float(egarch_fit_full.conditional_volatility.values[-1] ** 2)
            )
...
def run_aregarch(
    ...
    └─────────────────┴──────────────────────────────────────────────────┘
    Variance equation for both: EGARCH(p, o, q)
    """
    ...
    # ── FORECAST MODE ─────────────────────────────────────────────────
    if mode == "forecast":
        return _forecast_only_aregarch(params=params, horizon=horizon)
    ...
    model_label   = "ARX-EGARCH" if has_exogenous else "AR-EGARCH"
    ...
    log(f"   Variance model : EGARCH(p, o, q)")

    # ── Hyperparameters ───────────────────────────────────────────────
    split      = params.get("split",         0.85)
    use_auto   = params.get("auto_aregarch", True)
    diff_order = params.get("diff_order",    0)

    ar_lags   = params.get("ar_lags",   1)
    egarch_p  = params.get("egarch_p",  1)
    o_order   = params.get("egarch_o",  1)
    egarch_q  = params.get("egarch_q",  1)
    exog_lags = params.get("exog_lags", 1)
    arch_lags = params.get("arch_lags", 5)

    log("=" * 60)
    log(f"   auto_aregarch : {use_auto}")
    log(f"   diff_order    : {diff_order}")
    log(f"   Manual: ar_lags={ar_lags}, p={egarch_p}, o={o_order}, q={egarch_q}"
    ...
    # ── ARCH LM Test ──────────────────────────────────────────────────
    # NEW — use raw target series, same as app.py
    ...
    if not arch_significant:
        log("\n⚠️  ARCH LM test not significant — AR-EGARCH/EGARCH is not appropriate.")
        ...
        return {
            ...
            "arch_not_significant_message": (
                ...
                "EGARCH/AR-EGARCH cannot be applied without a valid pre-test. "
                ...
                f"{'AR-EGARCH' if has_exogenous else 'EGARCH'} is not an appropriate model "
    ...
    # ── AUTO-TUNE ─────────────────────────────────────────────────────
    if use_auto:
        best = auto_tune_aregarch(
            train_series, exog_train, has_exogenous,
            params, log_callback=log,
        )
        if best:
            ar_lags  = best["ar_lags"]
            egarch_p = best["egarch_p"]
            o_order  = best["egarch_o"]
            egarch_q = best["egarch_q"]
            log(f"\n✅ Best: {mean_spec}({ar_lags})-EGARCH({egarch_p},{o_order},{egarch_q})")
            params.update({
                "ar_lags"  : ar_lags,
                "egarch_p" : egarch_p,
                "egarch_o" : o_order,
    ...
    egarch_fit_train = model_train.fit(disp="off", show_warning=False)

    # ── In-sample (train) fitted values ───────────────────────────────
    fitted_mean = train_series.values - egarch_fit_train.resid.values
    valid_start = ar_lags
    train_pred  = fitted_mean[valid_start:]
    train_act   = train_series.values[valid_start:]
    train_t     = train_times.values[valid_start:]

    cond_vol      = egarch_fit_train.conditional_volatility.values[valid_start:]
    residuals_std = egarch_fit_train.resid.values[valid_start:] / (cond_vol + 1e-8)

    # ── Parameter estimates & information criteria ────────────────────
    #    Extracted from the TRAIN fit (best-order model)
    param_estimates_table = _param_estimates_df(egarch_fit_train)
    info_criteria         = _info_criteria(egarch_fit_train)
    ...
    if diff_order == 1:
        ...
        train_pred_= train_act_ - egarch_fit_train.resid.values[valid_start:]
    else:
    ...
    egarch_fit_full = model_full.fit(disp="off", show_warning=False)

    last_fitted_mean = float(series_aligned.iloc[-1] - egarch_fit_full.resid.values[-1])

    # ── Save bundle ───────────────────────────────────────────────────
    os.makedirs("static", exist_ok=True)
    n_exog_feats = int(exog_arr_all.shape[1]) if exog_arr_all is not None else 0

    with open("static/aregarch_model.pkl", "wb") as f:
        pickle.dump(egarch_fit_full, f)

    with open("static/aregarch_bundle.pkl", "wb") as f:
        pickle.dump({
            "ar_lags"         : ar_lags,
    ...
    title_str = (
        f"{mean_spec}({ar_lags})-EGARCH({egarch_p},{o_order},{egarch_q})"
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig("static/aregarch_train.png", dpi=120)
    plt.savefig("static/aregarch_test.png",  dpi=120)
    plt.close()
    ...
    return {
        ...
        "model_config": {
            ...
            "AR Lags"            : ar_lags,
            "p (ARCH Order)"     : egarch_p,
            "o (Leverage Order)" : o_order,
            "q (GARCH Order)"    : egarch_q,
