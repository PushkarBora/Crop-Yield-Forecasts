"""
MODWT-LSTM Hybrid Model  —  Multivariate + All PyWavelets Filters
==================================================================
Architecture mirrors wavelet_ann_model.py exactly, but replaces the
two-hidden-layer ANN with a per-coefficient LSTM + MC Dropout.

Workflow:
    y  →  MODWT  →  [W1, W2, …, WJ, VJ]
                         ↓  each coefficient
                    Grid Search best LSTM hyperparams
                    (features: coeff lags + raw exog lags as sequences)
                         ↓
                    Fit LSTM per coefficient
                         ↓
...
--------------------
  ...
  wavelet_ann_model.py — NO pywt.swt, NO level restrictions whatsoever.
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 1 — WAVELET UTILITIES  (identical to wavelet_ann_model.py)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 2 — LSTM ARCHITECTURE  (per-coefficient)
# ══════════════════════════════════════════════════════════════════════════════

class CoeffLSTM(nn.Module):
    """
    Small LSTM for one wavelet coefficient series.
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 3 — LAG FEATURE BUILDER  (3-D for LSTM)
# ══════════════════════════════════════════════════════════════════════════════
...
def _grid_search_lstm(series, exog, params, device, lags):
    """Grid-search LSTM hyperparameters for one wavelet coefficient."""
    ...
    grid = {
        "hidden_size"  : irange(params.get("ann_hidden_layer_1_min",  8),
                                 params.get("ann_hidden_layer_1_max",  16),
                                 params.get("ann_hidden_layer_1_step", 8)),
        "num_layers"   : irange(params.get("wlstm_num_layers_min",  1),
                                 params.get("wlstm_num_layers_max",  2),
                                 params.get("wlstm_num_layers_step", 1)),
        "dropout"      : frange(params.get("ann_dropout_1_min",  0.2),
    ...
    for p in combos:
        try:
            torch.manual_seed(SEED)
            m    = CoeffLSTM(F, p["hidden_size"],
                             p["num_layers"], p["dropout"]).to(device)
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 5 — FIT LSTM PER COEFFICIENT
# ══════════════════════════════════════════════════════════════════════════════

def _fit_lstm_on_coefficient(train_series, exog_train, params, device,
                              lags, best_p, coef_name, log):
    """
    Fit one LSTM on a single coefficient training series.

    Returns
    -------
    fitted_full : (N_train,)       first `lags` = actual coeff; rest = LSTM preds
    x_sc        : StandardScaler for X (2-D)
    y_sc        : StandardScaler for y
    model       : trained CoeffLSTM
    ...
    model = CoeffLSTM(F, best_p["hidden_size"],
                      best_p["num_layers"], best_p["dropout"]).to(device)
    ...
    for _ in range(best_p["epochs"]):
        ...
        if loss.item() < best_loss:
            ...
            counter += 1
            if counter >= best_p["patience"]: break

    model.load_state_dict({k: v.to(device) for k, v in best_state.items()})
    model.eval()

    log(f"  {coef_name} -> LSTM(hs={best_p['hidden_size']}, "
        f"nl={best_p['num_layers']}) | Loss: {best_loss:.4f}")
...
def _forecast_coefficient(model, x_sc, y_sc, last_win,
    ...
    last_win    : (lags, F) already scaled
    future_exog : (steps, n_exog) raw, or None
    """
    model.eval()
    F      = last_win.shape[1]
    n_exog = F - 1
...
def _run_one_combination(y_values, y_original, time_labels,
    ...
    """Full MODWT-LSTM pipeline for one (wavelet, level, lags) combo."""
    try:
        ...
        for name, train in zip(coef_names, train_coefs):
            best_p = _grid_search_lstm(train, exog_train, params, device, lags)
            lstm_configs[name] = best_p
            out = _fit_lstm_on_coefficient(
...
def _forecast_only_wavelet_lstm(params, horizon, n_samples=300):
    """Load saved bundle and generate MC Dropout interval forecast."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    bundle_path = "static/wavelet_lstm_bundle.pt"
    ...
    return {
        ...
        "model_name"    : f"Wavelet-LSTM ({wavelet.upper()}, L={level})",
    }
...
def run_wavelet_lstm(
    ...
    """
    MODWT-LSTM Hybrid Model.

    Param keys (all already collected by collect_hyperparameters):
        target_col, time_col, exog_cols, split
        wann_wavelets, wann_min_level, wann_max_level
        wann_lags_min, wann_lags_max, wann_lags_step
        ann_hidden_layer_1_{min,max,step}   -> LSTM hidden_size
        wlstm_num_layers_{min,max,step}     -> LSTM num_layers
        ann_dropout_1_{min,max,step}        -> LSTM dropout
    ...
    if mode == "forecast":
        return _forecast_only_wavelet_lstm(params, horizon)
    ...
    auto_tune     = params.get("auto_tune_wlstm", True)
    ...
    log(f"Wavelet-LSTM | {'Multivariate' if has_exogenous else 'Univariate'}")
    ...
    # Circular MODWT works at any level. Cap only at floor(log2(N)).
    ...
    if not all_results:
        raise RuntimeError("All wavelet-LSTM combinations failed.")
    ...
    torch.save(bundle_data, "static/wavelet_lstm_bundle.pt")
    ...
    ax.set_title(f"Wavelet-LSTM ({wavelet.upper()}, L={level}) — "
                 f"{'Multivariate' if has_exogenous else 'Univariate'}")
    ax.set_xlabel(time_col); ax.set_ylabel(target_col)
    ax.legend(); plt.tight_layout()
    plt.savefig("static/wavelet_lstm_train.png", dpi=120)
    plt.savefig("static/wavelet_lstm_test.png",  dpi=120)
    plt.close()
    ...
    model_config = {
        ...
        "lstm_configs"    : {
            ...
            for name, cfg in lstm_cfgs.items()
        },
